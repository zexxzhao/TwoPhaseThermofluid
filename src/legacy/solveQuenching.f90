      ...
      if (iand(assemble_tensor_flag, ASSEMBLE_TENSOR_MAT) > 0 .and. is_fluid) then
        do aa = 1, NSHL
          do bb = 1, NSHL
            ...
            xLSebe(aa, bb) = xLSebe(aa, bb)  + &
              (shlu(aa) + tauls * shconv(aa)) * fact2 * shlu(bb) * vdot * DetJ * gw(igauss) + &
              (shlu(aa) + tauls * shconv(aa)) * fact2 * phii * (1/rhoa - 1/rhow) * dmdphii * shlu(bb) * DetJ * gw(igauss) - &
              (shlu(aa) + tauls * shconv(aa)) * fact2 * dmdphii * shlu(bb) / rhoa * DetJ * gw(igauss)
            xLSebe(aa, bb) = xLSebe(aa, bb) + &
            ...
            xULSebe(:, aa, bb) = xULSebe(:, aa, bb) + &
              ...
              fact2 * shgradgu(aa, :) * divu * (-2d0/3d0) * dmudphi * shlu(bb) * DetJ * gw(igauss) + &
              fact2 * shgradgu(aa, :) * divu * drhodphi * tauC / rhoi * shlu(bb) * DetJ * gw(igauss)

            xULSebe(1, aa, bb) = xULSebe(1, aa, bb) + &
              fact2 * sum(shgradgu(aa, :) * (duidxi(1, :) + duidxi(:, 1))) * dmudphi * shlu(bb) * DetJ * gw(igauss)
            xULSebe(2, aa, bb) = xULSebe(2, aa, bb) + &
              fact2 * sum(shgradgu(aa, :) * (duidxi(2, :) + duidxi(:, 2))) * dmudphi * shlu(bb) * DetJ * gw(igauss)
            xULSebe(3, aa, bb) = xULSebe(3, aa, bb) + &
              fact2 * sum(shgradgu(aa, :) * (duidxi(3, :) + duidxi(:, 3))) * dmudphi * shlu(bb) * DetJ * gw(igauss)

            xPLSebe(aa, bb) = xPLSebe(aa, bb) - &
              fact2 * shlu(aa) * dmdphii * shlu(bb) * (1/rhoa - 1/rhow) * DetJ * gw(igauss) !- &
              !fact2 * sum(shgradgu(aa, :) * dpridxi(:)) * tauM / rhoi * drhodphi * shlu(bb) * DetJ * gw(igauss)
      ...
      if (iand(assemble_tensor_flag, ASSEMBLE_TENSOR_VEC) > 0 .and. is_fluid) then
        ...
        do aa = 1, NSHL
          RHSu(:, aa) = RHSu(:, aa) - &
            ...
            (mui + k_dc) * sum(shgradgu(aa, :) * (duidxi(:, 2) + duidxi(2, :))) * DetJ * gw(igauss) - &
            (mui + k_dc) * sum(shgradgu(aa, :) * (duidxi(:, 3) + duidxi(3, :))) * DetJ * gw(igauss) + &
...
!  write(*,*) "iel:",iel, NELEM
    ...
    do igauss = 1, NGAUSS
          ...
          do aa = 1, NSHL
            do bb = 1,NSHL
              xTebe(aa, bb) = xTebe(aa, bb) &
                + shlu(aa) * rhocpi * (fact1 * shlu(bb) + fact2 * shconv(bb)) * gw(igauss) * DetJ &
                - shlu(aa) * fact2 * dSedTi * shlu(bb) * gw(igauss) * DetJ &
                + tau_tem * rhocpi * shconv(aa) * rhocpi &
                    * (fact1 * shlu(bb) + fact2 * shconv(bb)) * gw(igauss) * DetJ &
                - tau_tem * rhocpi * shconv(aa) * fact2 * dSedTi * shlu(bb) * gw(igauss) * DetJ &
                + fact2 * (hki + k_dc_tem) * sum(shgradgu(aa, :) * shgradgu(bb, :)) * gw(igauss) * DetJ
      ...
      if (iand(assemble_tensor_flag, ASSEMBLE_TENSOR_VEC) > 0) then
        if(is_fluid) then
          RHSTem(:) = RHSTem(:) - shlu(:) * rhocpi * (rTi + dot_product(uadvi(:), dTdxi(:))) * gw(igauss) * DetJ
          RHSTem(:) = RHSTem(:) + shlu(:) * Se * gw(igauss) * DetJ
          RHSTem(:) = RHSTem(:) - rhocpi * tau_tem * shconv(:) * res_tem1 * gw(igauss) * DetJ
